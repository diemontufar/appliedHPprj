...
int     main(int argc, char** argv)
{
	...
	for(int m=0; m<N_p; m++)
	{
		phi[m]	= exp(-50*(pow(Points[m][0]-0.3, 2) + pow(Points[m][1]-0.3, 2)))+1.0;
	}
...
void	assemble(SparseMatrix& M, SparseMatrix& K, double* s, double* phi, bool* Free, bool* Fixed, double** Points, int** Faces, int** Elements, Boundary* Boundaries, int N_p, int N_f, int N_e, int N_b)
{
    ...
    for(int e=0; e<N_e; e++)
	{
		...
		gradEta[0][0] = (y[1]-y[2])/(2*Omega[e]); gradEta[0][1] = (y[2]-y[0])/(2*Omega[e]); gradEta[0][2] = (y[0]-y[1])/(2*Omega[e]);
		gradEta[1][0] = (x[2]-x[1])/(2*Omega[e]); gradEta[1][1] = (x[0]-x[2])/(2*Omega[e]); gradEta[1][2] = (x[1]-x[0])/(2*Omega[e]);
		...
		if		(Boundaries[b].type_=="neumann")
        {
			for(int f=0; f<Boundaries[b].N_; f++)
			{
				for(int p=0; p<2; p++)
				{
					...
					m			= Nodes[p];
					s[m]	   += mu*Boundaries[b].value_*Gamma[Boundaries[b].indices_[f]]/2;
				}
			}
		}
